...
#define ROW_MAXLINE 30
#define MAX_TABLE_COL 1024
...
void getGET(MYSQL *conn, int argc, char *argv[], char *content)
{
  ...
  int  isEnd = 1;
  char query[MAXLINE];
  char table[MAX_TABLE_COL][3][ROW_MAXLINE];
  int tableIndex = 0;
  ...
  if(isEnd == 1){
    ...
    while((row_sensor = mysql_fetch_row(sql_result_sensor))){
      strcpy(table[tableIndex][0],row_sensor[0]);
      strcpy(table[tableIndex][1],row_sensor[1]);
      strcpy(table[tableIndex][2],row_sensor[2]);
      tableIndex++;
    }

    if(argc == 2){
      sprintf(content,"%s%s\t%s\n",content,table[tableIndex-1][0],table[tableIndex-1][1]);
    } else { // add n option
      sscanf(argv[2],"count=%d",&count);
      if(tableIndex <= count) {
        for(int i = 0; i < tableIndex; i++ )
          sprintf(content,"%s%s\t%s\n",content,table[(tableIndex-1)-i][0],table[(tableIndex-1)-i][1]);
      }else {
